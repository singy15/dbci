...
namespace dbci
{
    public class ProcData
    {
        ...
        public int ExportParallel(string database, MemoryStream memoryStream, IDbConnection conn, IDbCommand cmd, string[] buffer, int[] rowsReady, int threadCount, int threadNumber, int[] threadStates, string encoding = "UTF-8")
        {
            using (var tx = conn.BeginTransaction())
            {
                var lineEndingChars = new char[] { '\r', '\n' };
                ...
                using (var csv = new CsvWriter(textWriter, config))
                {
                    ...
                    using (var reader = cmd.ExecuteReader())
                    {
                        ...
                        while (reader.Read())
                        {
                            ...
                            memoryStream.Seek((memoryStream.Position - lastPos) * (-1), SeekOrigin.Current);
                            string line = streamReader.ReadLine();
                            buffer[(rowsReady[threadNumber]) * threadCount + threadNumber] = line;
                            lastPos = memoryStream.Position;

                            rowsReady[threadNumber] = rowsReady[threadNumber] + 1;
                            csv.NextRecord();
                        }
